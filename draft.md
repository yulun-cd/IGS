...
##### PM~2.5~ and meteorological data
...
The hourly meteorological data, which includes barometric pressure, temperature, precipitation and wind speed, during the same period of time were also acquired from the London Air website. These data were used to calculate the hourly average barometric pressure, temperature and precipitation across London as the control variables of the study.

It is noteworthy that the PM~2.5~ and meteorological datasets used are all provisional (not ratified), so the result of this study should be evaluated and used with caution.
...
For the investigation of near-road greenspaces' effect on regional PM~2.5~ concentrations, this study focus on examining the relationship between the PM~2.5~ data from each air quality monitoring site and the near-road greenspace conditions in the 1km surrounding area of each site. The 1km buffer was decided based on some previous studies (Lei *et al.*, 2018; Chen *et al.*, 2019; Cai *et al.*, 2020) that investigated the effect of urban greenspaces on PM~2.5~. 

##### Data cleaning for PM~2.5~ and meteorological data

Before the analysis, the PM~2.5~ and meteorological datasets were cleaned. This includes (concrete process in the Appendix):

1. remove unusual values - some of the values that were very abnormal (e.g. -273 ℃, negative temperature during summer) were removed (set to be null).
2. fill missing values - then all the null values were filled using a technique called mean-before-after method (Norazian *et al.*, 2008), which is to replace a missing value with the mean of the data points before and after it. In cases where there were several continuous missing values, the closest non-null data points before and after the missing value were used to generate the replacement using linear regression. If the number of continuous missing values exceeded 12 (i.e. half a day), the 12 missing values were then replaced with the values from the same period of the previous day . This is a method commonly used in dealing with missing values within environmental datasets. 

After removing all outliers and filling all missing values, the meteorological datasets from different sites were then averaged and served as conditional variables for this study, which were used to indicate the weather condition of all air quality monitoring sites at a given time.

##### Spatial geometry manipulation

The 1km-radius buffers around all sites were generated based on site locations using Python package `geopandas`. Then all greenspaces and different types of roads in each site buffer were found using package `shapely`. The classification of roads contains the five types (include motorway) as mentioned above, as well as a sixth class 'Other' which represents all roads that are not assigned a road classification at national or local level (labelled 'Not Classified') or do not have the classification information (labelled 'Unknown') (OS, 2017).
...
One way to mitigate the influence from the number of roads is to divide the area of near-road greenspaces by the total length of roads. In this study, a greenspace-area-per-road-length number was calculated for each type of road as an indicator for the near-road greenspace conditions in each site buffer. This was accomplished based on the road and greenspace geometries prepared in the previous section.
